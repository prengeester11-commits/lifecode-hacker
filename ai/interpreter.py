...
4. 【쉬운 말 원칙 — 매우 중요, 현대 명리】 초등학교 고학년이 읽어도 술술 이해할 만큼 쉽게 씁니다.
   - 고전 명리 용어(상관·편재·식신·관성·인성·용신·기신 등)는 가능하면 '현대 일상어'로 먼저 풀고,
     꼭 필요할 때만 괄호로 원래 용어를 살짝 덧붙입니다.
     예: "내 마음을 표현하고 드러내야 풀리는 기운(옛말로는 상관)"
     예: "돈과 성과를 끌어오는 힘(재성)이 강해요"
     예: "나를 도와주고 채워주는 기운(인성)이 부족해요"
   - 어려운 한자어·추상어는 쓰지 말고, 생활 속 장면과 비유로 풀어줍니다.
     ("설기된다" 대신 "기운이 자꾸 빠져나간다", "왕하다" 대신 "세다/넘친다")
   - 한 문장은 짧게. 읽다가 숨차지 않게 끊어 씁니다.
   - 명리를 전혀 모르는 사람도 그 문단만 읽고 "아, 그런 뜻이구나" 하고 바로 넘어갈 수 있어야 합니다.
5. 추상적 표현 금지. 구체적 행동, 선택 기준, 관계 패턴으로 풀어냅니다.
6. 이분법적 단정 금지. "OO이다" 대신 "OO 경향이 있습니다", "OO로 나타날 수 있습니다" 사용.
7. 한국어 맞춤법 철저히 지키고, 오타가 없도록 주의합니다.

【십신(기운) 작용 기준 — 매우 중요, 계산 정확성】
- 십신은 시스템이 일간 기준으로 이미 정확히 계산해 제공합니다(천간은 그대로, 지지는 본기 기준).
  제공된 십신을 절대 스스로 다시 계산하거나 다른 십신으로 바꾸지 마세요. 제공된 값만 사용합니다.
- 각 십신이 일간에게 작용하는 결을 정확히 반영해 풀어주세요:
  · 비견·겁재(나와 같은 기운): 자립·주관·경쟁·동료. 겁재는 재물·관계에서 분산·경쟁이 강함.
  · 식신·상관(내가 내보내는 기운): 표현·창의·활동·자유. 상관은 톡톡 튀고 기존 틀에 반발하는 결.
  · 편재·정재(내가 다루는 기운): 재물·성과·현실 감각·이성. 정재는 안정적, 편재는 크고 유동적.
  · 편관·정관(나를 누르고 다스리는 기운): 책임·규율·압박·명예. 편관은 강한 자극·도전, 정관은 안정된 질서.
  · 편인·정인(나를 돕고 채우는 기운): 배움·수용·보호·생각. 편인은 직관·독창·고독, 정인은 따뜻한 보살핌.
- 천간(겉으로 드러나는 기운)과 지지(속에서 작동하는 기운)를 구분해, 두 기운이 어떻게 맞물리는지 자연스럽게 녹여주세요.
...
21. 그 사람의 사주 글자(일간·월지·용신·합충극 등)를 실제 근거로 들어 "그래서 당신은 이렇다"를 보여줍니다.
    근거가 보이면 "어떻게 이렇게까지 알지?" 하는 특별한 신뢰가 생깁니다.
...
【십신(十神) — 각 자리의 역할】
{json.dumps(saju_data['sipsin'], ensure_ascii=False)}
...
def generate_report_sections(saju_data: dict, astro_context: str = '') -> dict:
    ...
    prompts = [
        ...
        ('gyeokguk',     _prompt_gyeokguk(context)),       # 신설: 격국·용신
...
**코드 1. [키워드 두세 단어]**
[한 줄 핵심 설명. 그 뒤 5~7문장으로 이 에너지가 삶에서 어떻게 나타나는지 구체적 사례·맥락과 함께 서술. 명리 용어 사용 시 바로 옆에 쉬운 설명을 괄호로 추가]
...
def _prompt_gyeokguk(context: str) -> str:
    return f"""{context}

위 사주의 '격국(格局)과 용신(用神)'을 분석해 주세요.
이 섹션은 라이프코드 해커의 핵심 차별 섹션입니다. 의뢰인이 자기 사주의 본질 구조와 보충해야 할 에너지를 명확히 이해할 수 있도록 풀어주세요.

분량: 약 1300~1500자.

형식:

## 내 사주의 본질 구조와 보충해야 할 에너지

**1. 격국(格局): 이 명식의 기본 캐릭터**
[월령(月令) 본기를 기준으로 정재격/편재격/식신격/상관격/정관격/편관격/정인격/편인격 중 어디에 해당하는지 명확히 판정하고, 그 격국이 가진 본래의 성정과 강점, 그리고 잘 풀렸을 때의 모습을 5~7문장으로]

**2. 신강/신약 한 번 더 정리**
[월령·득지·득세 결과를 일상어로 풀어 설명. "월지가 일간을 도와주지 않았다"가 무엇을 뜻하는지 의뢰인의 실제 삶 감각으로 해석. 3~5문장]

**3. 용신(用神): 당신에게 부족한 에너지**
[신강이면 식상/재성/관성, 신약이면 비겁/인성. 어느 오행이 보충되어야 하는지 정확히 짚고, 그 오행이 의뢰인의 삶에 어떻게 작용하면 좋은지 5~7문장으로 풀어주세요]

**4. 기신(忌神): 잠시 거리를 두면 좋은 에너지**
[용신 반대 방향. 어떤 행동·환경이 의뢰인의 에너지를 빼앗는지 구체적으로. 4~6문장]

**5. 라이프스타일 처방**
[용신 오행에 맞는 색깔, 방향, 음식, 환경, 활동을 구체적으로 안내. 토 보충이면 황색·중부 지방·뿌리 채소·안정된 공간, 화 보충이면 빨강·남쪽·따뜻한 음식·활동적 공간 등. 4~6항목]

**6. 닫는 위로 (2~3문장)**
[보충해야 할 에너지가 있다는 건 결핍이 아니라 성장 방향이라는 점을 따뜻하게 전해주세요]
"""
...
def _prompt_daewoon(context: str, saju_data: dict) -> str:
    ...
    cur_line = ''
    if cur:
        cur_line = (f"\n[현재 대운 십신 — 이미 계산됨, 그대로 사용]\n"
                    f"- 현재 대운 {cur['name']}: 천간 {cur.get('stem_sipsin','')} · 지지(본기) {cur.get('branch_sipsin','')}\n")
    return f"""{context}
...
**현재 대운의 에너지 (6~8문장)**
[현재 대운이 어떤 에너지인지, 이 시기의 주제와 과제. 일간과 만나면 어떤 십신 관계인지 명확히 짚어주기]
...
def _prompt_sewoon(context: str, saju_data: dict) -> str:
    ...
    s1s, s1b = sw['first'].get('stem_sipsin', ''), sw['first'].get('branch_sipsin', '')
    s2s, s2b = sw['second'].get('stem_sipsin', ''), sw['second'].get('branch_sipsin', '')
    return f"""{context}

{y1}년({n1}년)과 {y2}년({n2}년) 세운이 이 사람의 원국과 어떻게 작용하는지 분석해 주세요.

[십신은 이미 계산됨 — 그대로 사용하고 추측·변경 금지]
- {y1}년 {n1}: 천간 {s1s} · 지지(본기) {s1b}
- {y2}년 {n2}: 천간 {s2s} · 지지(본기) {s2b}
...
[{y1}년 세운이 이 사람의 원국, 대운과 어떻게 만나는지. 일간과의 십신 관계 포함]
...
def _prompt_calendar(context: str, saju_data: dict) -> str:
    ...
    # 간지 + 십신을 코드로 미리 계산해 그대로 제공 (GPT가 직접 계산하면 틀림)
    month_lines = '\n'.join(
        f"  {mp['month']}월 {mp['name']}({mp['name_hanja']})월: 천간 {mp['stem_sipsin']} · 지지 {mp['branch_sipsin']}"
        for mp in mps
    )
    month_blocks = '\n\n'.join(
        f"**{mp['month']}월 ({mp['name']}·{mp['name_hanja']}) - [이 달의 키워드 두 단어]**\n"
        f"[이 달은 천간 {mp['stem_sipsin']}, 지지 {mp['branch_sipsin']}의 기운입니다. "
        f"이 십신이 일간에게 실제로 어떻게 작용하는지를 바탕으로 추천 행동과 한 호흡 고를 행동을 3~4문장으로. "
        f"십신은 이미 확정되었으니 절대 바꾸지 마세요.]"
        for mp in mps
    )
    return f"""{context}

위 사주를 바탕으로 '{cal_year}년 12개월 운세 캘린더'를 작성해 주세요.

[매우 중요 — 십신은 이미 정확히 계산되었습니다]
아래는 {cal_year}년 각 월의 월주 간지와, 그 천간·지지가 의뢰인 일간과 맺는 '십신'입니다.
이 십신을 절대 임의로 바꾸거나 추측하지 말고, 제공된 십신을 그대로 사용해 해석하세요.
(지지의 십신은 그 지지의 본기 기준으로 이미 산출되어 있습니다.)
{month_lines}
